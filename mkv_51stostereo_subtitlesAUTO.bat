...
echo Jimmy's - ffmpeg: '5.1(side) to Stereo' AND 'Subtitles' AUTOMATED converter
@echo:
@echo:
...
for %%a in ("%fromFolder%") do ffmpeg -i "%fileToConvert%" -vf "subtitles="%subtitleFileConvert%.ass":force_style='PrimaryColour=&HFFFFFF&,BorderStyle=3'" -acodec aac -af:a:0 "pan=stereo|FL=FC+0.50*FL+0.50*SL+0.50*LFE|FR=FC+0.50*FR+0.50*SR+0.50*LFE" -metadata title="%fileOutputName%" "%fileOutputName%.mp4"
...
echo Copy to NAS - Movies? Press any key to transfer file. Press 'Crtl' + 'c' to cancel transfer.
pause
