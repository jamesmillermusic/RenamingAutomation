...
echo Jimmy's - ffmpeg: '5.1 to Stereo' AUTOMATED converter
@echo:
@echo:
...
for %%a in ("%fromFolder%") do ffmpeg -i "%fileToConvert%" -vcodec copy -acodec aac -af:a:0 "pan=stereo|FL=FC+0.50*FL+0.50*BL+0.50*LFE|FR=FC+0.50*FR+0.50*BR+0.50*LFE" -metadata title="%fileMetadataName%" "%fileOutputName%.mp4"
