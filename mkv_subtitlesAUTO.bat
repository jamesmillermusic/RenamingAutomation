REM Subtitles compontent for ffmpeg AUTOMATED process
echo 'Subtitles' compontent for ffmpeg AUTOMATED process
...
echo Jimmy's - ffmpeg: 'Subtitles' converter
@echo:
@echo:
...
for %%a in ("%fromFolder%") do ffmpeg -i "%fileToConvert%" -vf "subtitles="%subtitleFileConvert%.ass":force_style='PrimaryColour=&HFFFFFF&,BorderStyle=3'" -acodec copy -metadata title="%fileOutputName%" "%fileOutputName%.mp4"
