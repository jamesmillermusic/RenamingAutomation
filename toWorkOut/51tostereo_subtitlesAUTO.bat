...
echo Jimmy's - ffmpeg: '5.1 to Stereo' AND 'Subtitles' AUTOMATED converter
@echo:
@echo:
...
for %%a in ("%fromFolder%") do ffmpeg -i "%fileToConvert%" -vf "subtitles="%subtitleFileConvert%.ass":force_style='PrimaryColour=&HFFFFFF&,BorderStyle=3'" -acodec aac -af:a:0 "pan=stereo|FL=FC+0.50*FL+0.50*BL+0.50*LFE|FR=FC+0.50*FR+0.50*BR+0.50*LFE" -metadata title="%fileOutputName%" "%fileOutputName%.mp4"
